...
//        })
        ...
        checkoutViewModel.paymentIntentClientSecretLiveData.observe(viewLifecycleOwner, {
            when (it) {
                ...
                is Resource.Error -> {
                    showToast("Please  First add Products to Cart")
                    closeFragment()
                }
                is Resource.Loading -> {
    ...
    private fun onPaymentResult(paymentResult: PaymentResult) {
        val isOrderSubmitted = when (paymentResult) {
            is PaymentResult.Completed -> false
            else -> false
    ...
    private fun navigateToOrderStatusFragment() {
        val action =
            CheckoutFragmentDirections.actionCheckoutFragmentToOrderStatusFragment(orderModel, true)
